...
<div class="student-panel mb-6">
    <div class="flex flex-wrap items-end justify-between gap-4">
        <div>
            <p class="text-label">Physical class schedule</p>
            <h2 class="font-display text-2xl mt-1">Your course calendar</h2>
            <p class="mt-2 text-[var(--color-soft-grey)]">Your academy class appointments, practical sessions and assessments across every enrolled course.</p>
            <div class="mt-3 flex flex-wrap gap-2">
                @foreach($enrolments as $courseEnrolment)
                    <span class="student-badge">{{ $courseEnrolment->course?->name }}</span>
                @endforeach
            </div>
        </div>
...
<div class="grid gap-4 mb-8">
    @forelse($upcomingSessions as $session)
        <article class="student-panel grid gap-4 md:grid-cols-[8rem_1fr_auto] md:items-center">
            ...
            <div><div class="flex flex-wrap gap-2 mb-2"><span class="student-badge">{{ $session->schedule?->course?->name }}</span> @if($session->is_practical)<span class="student-badge">Practical</span>@endif @if($session->is_assessment)<span class="student-badge">Assessment</span>@endif <span class="student-badge">{{ ucfirst($session->status) }}</span></div><h3 class="font-display text-xl">{{ $session->topic ?: 'Training session' }}</h3><p class="mt-1">{{ $session->starts_at ? substr($session->starts_at,0,5) : 'Time TBC' }}@if($session->ends_at) – {{ substr($session->ends_at,0,5) }}@endif</p>@if($session->location)<p class="text-sm text-[var(--color-soft-grey)] mt-1">Location: {{ $session->location }}</p>@endif @if($session->announcement)<p class="mt-3">{{ $session->announcement }}</p>@endif</div>
